...
| `src/auth.js` | **Dashboard login gate** (`requireAuth()`), called before `main()` renders. Username `energydrinks` (plaintext compare); password checked against a SHA-256 hash (`51399b…`, = `energydrinks12345`); session flag in `sessionStorage`. **Client-side deterrent only — NOT real security** (creds + data ship in the bundle, bypassable via devtools). For real protection use nginx Basic Auth / Cloud Run IAP. To change the password: `printf '%s' 'NEWPASS' \| sha256sum` and update `PASS_HASH`. |
