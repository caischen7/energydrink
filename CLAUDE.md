...
| `src/style.css` | The **playful banana** design system — CSS custom properties sampled from the can artwork: `--cream` #fff6df page field, `--peel` #ffd23f banana yellow, `--zest` #ef4a23 mascot red-orange accent, `--plum` #421a39 midnight-berry close, `--ink` #2e1b0c peel-brown. Type: chunky expanded **Archivo** (width axis) display, **Space Grotesk** body, **Space Mono** telemetry. Signatures: mega hero with the 2nd word peeled to an outline (`-webkit-text-stroke`), hand-drawn SVG squiggle section dividers, and flavor rows that flood with their can's colorway on hover. Honors reduced-motion + no-WebGL. Shared by both pages. (`--volt`/`--accent` are legacy hooks → `--zest`.) |
